...
export default function MyList() {
  ...
  // Watchlist remove with confirm dialog (demotrating ON DELETE CASCADE + trg_review_cleanup)
  const handleRemove = async (animeId, title) => {
    const ok = confirm(
      `Remove "${title}" from your watchlist?\n\nNOTE: Database trigger trg_review_cleanup will ON DELETE CASCADE your review for this anime!`
    );
    if (!ok) return;
    ...
    <div className="space-y-10 py-6">
      ...
      <section className="space-y-4">
        <div className="flex items-baseline justify-between">
          ...
          <Concept>Rows generated by DB AFTER trigger fn_log_watchlist</Concept>
        </div>
