...
export const Api = {
  ...
  watchlist: (params) => api.get("/watchlist/me", { params }).then((r) => r.data),
  ...
  reviews: (animeId, sort) => api.get(`/reviews/anime/${animeId}`, { params: { sort } }).then((r) => r.data),
  ...
  years: () => api.get("/anime/years").then((r) => r.data),
  statuses: () => api.get("/anime/statuses").then((r) => r.data),
  search: (q, limit = 10) => api.get("/anime/search", { params: { q, limit } }).then((r) => r.data),
  randomAnime: () => api.get("/anime/random").then((r) => r.data),
  similar: (id, limit = 8) => api.get(`/anime/${id}/similar`, { params: { limit } }).then((r) => r.data),
  scoreDistribution: (id) => api.get(`/anime/${id}/score-distribution`).then((r) => r.data),
  userDirectory: (params) => api.get("/users", { params }).then((r) => r.data),
  updateProfile: (patch) => api.patch("/users/me", patch).then((r) => r.data),
  userWatchlist: (id, status) =>
    api.get(`/users/${id}/watchlist`, { params: { status } }).then((r) => r.data),
  userReviews: (id) => api.get(`/users/${id}/reviews`).then((r) => r.data),
